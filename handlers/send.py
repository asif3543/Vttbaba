...
try:
    ...
    if bot_member.status not in ["administrator", "creator"]:
        ...
        await message.reply(
            "❌ I am not admin in that channel."
        )
...
for ch in channels:
    ...
    keyboard.append([
        InlineKeyboardButton(
            text=ch["name"],
            callback_data=f"single_{ch['_id']}"
        )
    ])
...
for ch in channels:
    ...
    checked = (
        "✅"
        if str(ch["_id"]) in multi_selected[uid]
        else "☐"
    )
...
await callback.message.reply(
    f"✅ {len(selected)} channel(s) selected.\nType `/confirm`"
)
...
# MULTI SEND
    ...
    await message.reply(
        f"✅ Sent to {success}/{total} channels"
    )
