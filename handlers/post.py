...
if message.photo or message.video or message.document or message.forward_from_chat:
    ...
    await db.save_temp(
        ...
        {
            **temp,
            "episode_msg_id": stored_ep.message_id
        }
    )
...
if message.text == "done":
    ...
    if len(episodes) < 2:
        ...
        await message.reply(
            "❌ Minimum 2 episodes required."
        )

        return

    await message.reply(
        "Send range (example: `05-15`)"
    )
...
keyboard = InlineKeyboardMarkup(
    inline_keyboard=[
        [
            InlineKeyboardButton(
                text="📤 Send",
                callback_data="send_single"
            )
        ],
        [
            InlineKeyboardButton(
                text="📤 Send more channel",
                callback_data="send_multi"
            )
        ]
    ]
)
