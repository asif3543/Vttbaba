...
async def single_ep(message: Message, state: FSMContext):
    ...
    if message.photo or message.video or message.document or message.forward_from_chat:
        ...
        await db.save_temp(
            message.from_user.id,
            {**temp, "episode_msg_id": stored_ep.message_id}
        )
        await message.reply(
...
async def confirm_post(message: Message):
    ...
    button = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text=btn_text, url=deep_link)]
        ]
    )
    ...
    # SAVE POST (FIXED - with your requested change)
    await db.save_post({
        "storage_msg_id": temp.get("episode_msg_id", temp["storage_msg_id"]),
        "type": temp.get("type"),
